...
try:
    ...
    from trt_utils_benchmark import load_engine, build_engine, save_engine
except ImportError:
    raise ImportError(
...
class CLIPTensorRTModel:
    ...
    def  start_engines(self,use_fp16=False):
        ...
        # if compute_capacity != (8, 6):
            ...
            # )
        from clip_onnx_benchmark import CLIPOnnxModel
    ...
    def encode_image(self, onnx_image):
        (visual_output, avg_time) = self._visual_engine({'input': onnx_image})

        return visual_output, avg_time

    def encode_text(self, onnx_text):
        (textual_output, avg_time) = self._textual_engine({'input': onnx_text})

        return textual_output, avg_time